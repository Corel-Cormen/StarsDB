...
def insert_constellation(cursor, name):
    cursor.execute("INSERT INTO Constellation (name) VALUES (%s) RETURNING id;", (name,))
    return cursor.fetchone()['id']

def insert_solar_system(cursor, name):
    cursor.execute("INSERT INTO SolarSystem (name) VALUES (%s) RETURNING id;", (name,))
    return cursor.fetchone()['id']
...
def insert_location(cursor, system_id, loc, constellation_id):
    cursor.execute("""
        ...
        VALUES (%s,%s,%s,%s,%s,%s)
    """, (system_id, loc.get("RightAscensionNum"), loc.get("DeclinationNum"), loc.get("Parallax"),
          loc.get("SunDistance"), constellation_id))

def insert_star(cursor, name, system_id):
    cursor.execute("INSERT INTO Star (name, id_system) VALUES (%s, %s) RETURNING id;", (name, system_id))
    return cursor.fetchone()['id']
...
def insert_star_type(cursor, name):
    cursor.execute("INSERT INTO StarType (TypeName) VALUES (%s) RETURNING id;", (name,))
    return cursor.fetchone()['id']

def insert_star_subtype(cursor, name):
    cursor.execute("INSERT INTO StarSubtype (TypeName) VALUES (%s) RETURNING id;", (name,))
    return cursor.fetchone()['id']
...
def insert_planet_type(cursor, name):
    cursor.execute("INSERT INTO PlanetType (TypeName) VALUES (%s) RETURNING id;", (name,))
    return cursor.fetchone()['id']

def insert_planet_subtype(cursor, name):
    cursor.execute("INSERT INTO PlanetSubtype (TypeName) VALUES (%s) RETURNING id;", (name,))
    return cursor.fetchone()['id']
